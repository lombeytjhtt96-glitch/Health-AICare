#!/usr/bin/env bash
# Development helper script for Health-AICare
...
resolve_python() {
  ...
  local candidates=(
    ...
    "$PROJECT_DIR/backend/venv/bin/python"
    "$PROJECT_DIR/backend/.venv/Scripts/python.exe"
    "$PROJECT_DIR/backend/.venv/bin/python"
...
cleanup_all() {
  ...
  if [[ -n "$AI_PID" ]]; then
    kill_process_tree "$AI_PID" "ai-engine"
  fi
...
cleanup_stale_processes() {
  ...
  if [[ -f "$PID_FILE" ]]; then
    ...
    if [[ -n "${AI_PID:-}" ]]; then
      kill_process_tree "$AI_PID" "stale ai-engine" 2>/dev/null || true
    fi
...
monitor_processes() {
  ...
  while true; do
    ...
    if [[ -n "$AI_PID" ]] && ! kill -0 "$AI_PID" 2>/dev/null; then
      log_error "AI Engine process died unexpectedly!"
      cleanup_all
      exit 1
    fi
  done
...
run_local() {
  ...
  local ai_env_file="$PROJECT_DIR/ai/.env"
  ...
  ai_port="$(read_env_var "$ai_env_file" "PORT" "${AI_EXTERNAL_PORT:-8080}")"
  ...
  local ai_origin="http://localhost:${ai_port}"
  ...
  log_info "Cleaning up any existing processes on ports ${backend_port}, ${frontend_port}, ${docs_port}, and ${ai_port}..."
  kill_port_processes "$backend_port"
  kill_port_processes "$frontend_port"
  kill_port_processes "$docs_port"
  kill_port_processes "$ai_port"
  ...
  if [[ "$use_separate_terminals" == "true" ]]; then
    run_separate_terminals "$python_bin" "$backend_port" "$frontend_port" "$docs_port" "$ai_port" "$backend_env_file" "$frontend_env_file" "$docs_env_file" "$ai_env_file" "$backend_origin" "$frontend_origin" "$docs_origin" "$ai_origin"
  else
    run_inline "$python_bin" "$backend_port" "$frontend_port" "$docs_port" "$ai_port" "$backend_env_file" "$frontend_env_file" "$docs_env_file" "$ai_env_file" "$backend_origin" "$frontend_origin" "$docs_origin" "$ai_origin"
  fi
}
...
run_inline() {
  ...
  local frontend_env_file="$7"
  local docs_env_file="$8"
  local ai_env_file="$9"
  local backend_origin="${10}"
  local frontend_origin="${11}"
  local docs_origin="${12}"
  local ai_origin="${13}"
  ...
  # Brief delay to let docs start binding
  sleep 2

  # Start AI Engine in background with process group
  (
    cd "$PROJECT_DIR/ai"
  ...
  AI_PID=$!
  log_ok "AI Engine started (PID: $AI_PID)"
...
run_separate_terminals() {
  ...
  # Use mintty for Git Bash to avoid WSL path conflicts under Windows Terminal.
  if false && is_windows_shell && (has_cmd wt.exe || has_cmd wt); then
...
echo ""
echo "Health-AICare Backend Server"
echo "=============================="
...
echo "Health-AICare Frontend Server"
echo "=============================="
...
echo "Health-AICare Docusaurus Docs"
echo "=============================="
...
echo ""
echo "Health-AICare AI Engine RAG Server"
echo "=============================="
...
EOF
    ...
    chmod +x "$backend_script" "$frontend_script" "$docs_script" "$ai_script"
    ...
    "${wt_cmd[@]}" -w 0 \
      new-tab --title "Health-AICare Backend" bash "$backend_script" \; \
      new-tab --title "Health-AICare Frontend" bash "$frontend_script" \; \
      new-tab --title "Health-AICare Docs" bash "$docs_script" \; \
      new-tab --title "Health-AICare AI Engine" bash "$ai_script" &
    ...
    echo ""
    log_ok "Opened backend + frontend + docs + AI Engine in Windows Terminal tabs."
    log_info "To stop them: close the panes/windows, or run ./run_dev.sh stop"

    (sleep 10 && rm -f "$backend_script" "$frontend_script" "$docs_script" "$ai_script") &
    return 0
  fi
...
EOF
    ...
    mintty --title "Health-AICare Backend" --exec bash "$backend_script" &
    BACKEND_PID=$!

    mintty --title "Health-AICare Frontend" --exec bash "$frontend_script" &
    FRONTEND_PID=$!

    mintty --title "Health-AICare Docs" --exec bash "$docs_script" &
    DOCS_PID=$!

    mintty --title "Health-AICare AI Engine" --exec bash "$ai_script" &
    AI_PID=$!

    save_pids

    echo ""
    log_ok "Started backend, frontend, docs, and AI Engine in separate terminals."
    echo ""
    echo "   Backend:   http://localhost:${backend_port}"
    echo "   Frontend:  http://localhost:${frontend_port}"
    echo "   Docs:      http://localhost:${docs_port}"
...
show_help() {
  cat << 'EOF'
Health-AICare Development Script

Usage: ./run_dev.sh <command> [args]

Main Commands:
  local                    Start backend + frontend + docs + AI Engine locally (unified terminal)
  local --separate         Start backend + frontend + docs + AI Engine in separate terminals (Windows Terminal if available)
...
Examples:
  ./run_dev.sh local          # Start local dev servers (backend/frontend/docs/AI)
