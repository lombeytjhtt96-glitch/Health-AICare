...
async def handle_case_created_event(event: Event) -> None:
    ...
    try:
        ...
        # Broadcast as case_created for Quick Triage
        await broadcaster.broadcast(
            ...
            data={
                ...
                "severity": severity,
                "status": "new",
                "sla_breach_at": event.data.get("sla_breach_at"),
                "created_at": datetime.utcnow().isoformat()
            }
        )
        
        logger.info(f"Broadcasted case_created event and alert for case {case_id}")
...
async def handle_case_status_changed_event(event: Event) -> None:
    ...
    try:
        ...
        if (old_status, new_status) in significant_changes:
            # Broadcast as case_updated for dashboards
            await broadcaster.broadcast(
            ...
            logger.info(f"Broadcasted case_updated event for case {case_id}")
