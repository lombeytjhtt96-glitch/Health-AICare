...
The system has four canonical roles:
    ...
    user         any authenticated app user (people, lecturers, staff)
...
frontend builds are silently resolved to their canonical equivalents:
    ...
    "user"                            → user       (renamed; lecturers also use the app)
...
ROLE_USER = "user"  # Any authenticated app user — people, lecturers, staff
...
_ALIAS_MAP: dict[str, str] = {
    # user aliases
    "user": ROLE_USER,   # legacy name — renamed because lecturers also use the app
    # counselor aliases
