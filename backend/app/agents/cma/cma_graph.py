...
async def ingest_escalation_node(state: SDAState) -> SDAState:
    ...
    if severity not in ("med", "moderate", "high", "critical"):
        errors = state.get("errors", [])
        errors.append(
            f"CMA should only handle med/high/critical cases, got severity={severity}"
        )
        state["errors"] = errors
        if execution_id:
...
async def schedule_appointment_node(state: SDAState, config: RunnableConfig) -> SDAState:
    ...
    based on user preferences, counselor availability, and case context.
    ...
    5. Use LLM to select best slot matching user preferences
...
async def _select_optimal_psychologist(
    ...
    Args:
        ...
        preferences: User preferences (specialization, language, etc.)
...
Konteks Case:
- Severity: {severity}
- Preferensi Semua Orang: {json.dumps(preferences)}
...
2. Match specialization kalau user punya concern spesifik
...
async def _find_optimal_appointment_time(
    ...
    Args:
        ...
        preferred_time: User's time preference
...
{urgency_text}
Preferensi Semua Orang: {preferred_time or 'Nggak ada yang specified'}
