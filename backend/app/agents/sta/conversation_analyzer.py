...
async def analyze_conversation_risk(
    ...
    if not conversation_history:
        raise ValueError("Cannot analyze empty conversation - history is missing")
...
4. USER CONTEXT: Extract specific information about:
   - recent_stressors: What problems/challenges are they facing? (CRITICAL: Use concise keywords, max 2-3 words per item, e.g., "Work Pressure", "Family Conflict")
   - coping_mechanisms: How do they currently handle stress? (Max 2-3 words, e.g., "Exercise", "Socializing")
   - protective_factors: What positive supports do they have? (Max 2-3 words, e.g., "Close Friends", "Religious Faith")

5. CONCERNS: What specific patterns are worrying? (CRITICAL: Use concise keywords, max 3 words, e.g., "Sleep Deprivation", "Social Withdrawal")
...
- academic: Study/career-specific - Study/work pressure, fear of failure, job search/task stress
