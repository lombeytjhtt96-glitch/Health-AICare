...
async def _ensure_langgraph_execution_row(
    ...
    *,
    ...
    try:
        ...
        # Another task inserted it while we were working - this is expected in background task races
        await db.rollback()
        # The row now definitely exists, so we can return
        return
    ...
    def record_decision_parse_outcome(
        ...
        *,
        ...
        self.record_custom_metric(
            ...
            node_id="health_ai_decision",
        )

        if initial_parse_failed:
            self.record_custom_metric(
                execution_id,
                "health_ai_decision_parse_failed",
                1.0,
                metric_unit="count",
                metric_category="quality",
                node_id="health_ai_decision",
        ...
        if repaired:
            self.record_custom_metric(
                execution_id,
                "health_ai_decision_parse_repaired",
                1.0,
                metric_unit="count",
                metric_category="quality",
                node_id="health_ai_decision",
        ...
        if initial_parse_failed and not repaired:
            self.record_custom_metric(
                execution_id,
                "health_ai_decision_parse_unrecovered",
                1.0,
                metric_unit="count",
                metric_category="quality",
                node_id="health_ai_decision",
    ...
    async def _check_decision_parse_failure_threshold(self, *, days: int = 1) -> None:
        ...
        try:
            ...
            await self._create_alert(
                ...
                metric_name="health_ai_decision_parse_failure_rate_percent",
            )
        except Exception as e:
    ...
    async def get_decision_parse_health(self, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            async with AsyncSessionLocal() as db:
                ...
                attempts = int(await _sum_metric("health_ai_decision_parse_attempt"))
                failed = int(await _sum_metric("health_ai_decision_parse_failed"))
                repaired = int(await _sum_metric("health_ai_decision_parse_repaired"))
                unrecovered = int(await _sum_metric("health_ai_decision_parse_unrecovered"))
