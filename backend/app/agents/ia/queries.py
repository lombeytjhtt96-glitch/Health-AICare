...
ALLOWED_QUERIES: Final[dict[QuestionId, str]] = {
        ...
        SELECT 
            ...
            COUNT(DISTINCT CASE 
                WHEN (
                    SELECT COUNT(*) 
                    FROM messages m 
                    WHERE m.session_id::TEXT = c.session_id::TEXT
                ) <= 2 
                THEN c.id 
    ...
    "cost_per_helpful": """
        SELECT 
            ...
            COUNT(DISTINCT CASE 
                WHEN ta.severity_level IN ('low', 'med')
                AND EXISTS (
    ...
    "coverage_windows": """
        SELECT 
            ...
            EXTRACT(DOW FROM c.timestamp) as day_of_week,  -- 0=Sunday, 6=Saturday
    ...
    "topic_analysis": """
        ...
        FROM (
            SELECT 
                json_array_elements_text(user_context->'recent_stressors') as topic,
                user_id
            FROM conversation_risk_assessments
            WHERE 
                created_at >= :start_date 
                AND created_at < :end_date
                AND user_context->'recent_stressors' IS NOT NULL
        ) as subquery
        GROUP BY topic
    ...
    "intervention_latency": """
        SELECT 
            DATE(ta.created_at) as date,
            AVG(EXTRACT(EPOCH FROM (ta.created_at - (c.timestamp AT TIME ZONE 'UTC')))) as avg_latency_seconds,
