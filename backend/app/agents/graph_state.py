...
class HealthAIOrchestratorState(TypedDict, total=False):
    """State for the unified HealthAI orchestrator graph.
    
    This state flows through the new LangGraph workflow where HealthAI is the
    ...
    # ============================================================================
    # HealthAI DECISION NODE OUTPUTS
    # ============================================================================
    intent: Optional[str]
    ...
    """Direct conversational response from HealthAI (when needs_agents=False)."""
    ...
    # Analytics Query Params (parsed by HealthAI)
    ...
    """List of tool calls generated by HealthAI."""
    ...
    # Tier 1: Per-message immediate risk screening (from HealthAI's JSON)
    immediate_risk_level: Optional[Literal["none", "low", "moderate", "high", "critical"]]
    """Immediate risk detected by HealthAI in current message via JSON output."""
    ...
    """Brief explanation from HealthAI about why this risk level was assigned."""
    ...
    """Final synthesized response to user (either from HealthAI or agents)."""
