...
The sole exception is ``get_health_ai_system_prompts()``, which performs a lazy
import of ``app.agents.health_ai.identity`` to avoid the circular-import chain
that arises when the module loads alongside the rest of the agent package.
That import is idempotent and causes no I/O.

Public API
----------
get_health_ai_system_prompts       Return the role-keyed system prompt dictionary.
...
def get_health_ai_system_prompts() -> dict[str, str]:
    """Return the role-keyed system prompt dictionary.

    Performs a lazy import of ``app.agents.health_ai.identity`` to avoid the
    circular-import chain that arises at module load time.  Falls back to
    minimal inline prompts when the import fails.
    """
    try:
        from app.agents.health_ai.identity import HEALTH_AI_SYSTEM_PROMPTS  # type: ignore[import]
        return HEALTH_AI_SYSTEM_PROMPTS
    except ImportError:
...
def format_personal_memory_block(state: HealthAIOrchestratorState) -> str:
...
# These let health_ai_orchestrator_graph and its dependants keep existing call
# sites while the orchestrator is migrated incrementally.  They will be
# removed once the orchestrator is fully decomposed.
# ---------------------------------------------------------------------------
_get_health_ai_prompts = get_health_ai_system_prompts
