...
----------------------------------------
The public entry point is ``health_ai_decision_node``.  All heavy logic is
...
Coordinator:
    health_ai_decision_node           Orchestrates the above helpers in sequence.
"""
...
from app.agents.health_ai.constants import MAX_HISTORY_TURNS as _MAX_HISTORY_TURNS
...
class _DirectResponseResult(NamedTuple):
    ...
    response_text: str
    tool_calls: list[Any]
    response_source: Literal["health_ai_direct", "health_ai_react_tools"]
    preferred_model: str
    ...
    try:
        ...
        if policy_result.decision.value == "deny":
            patches.update({
                ...
                "health_ai_direct_response": _AUTOPILOT_DENIED_RESPONSE,
            })
...
async def _apply_screening_discordance_policy(
    ...
    try:
        from app.agents.health_ai.screening_awareness import (
...
async def _generate_direct_response(
    ...
    return _DirectResponseResult(
        ...
        response_source="health_ai_react_tools" if tool_calls else "health_ai_direct",
...
async def _record_decision_audit(
    ...
    try:
        from app.domains.mental_health.services.agent_decision_audit_service import (
            record_health_ai_decision_event,
        )
        event = await record_health_ai_decision_event(db, state, raw_decision=raw_decision)
        return int(event.id)
    except Exception as exc:
    ...
    try:
        ...
        if normalized_role == "user" and _is_smalltalk_message(current_message):
            cast(dict[str, Any], state).update({
                ...
                "health_ai_direct_response": _build_smalltalk_response(normalized_role),
                "response_source": "health_ai_direct",
            })
            state["final_response"] = state.get("health_ai_direct_response")
            state.setdefault("execution_path", []).append("health_ai_decision")
        ...
        try:
            ...
            if repaired_decision is not None:
                ...
                if holding_response and not state.get("health_ai_direct_response"):
                    state["health_ai_direct_response"] = holding_response
                ...
                if not state.get("needs_agents"):
                    ...
                    state["health_ai_direct_response"] = result.response_text
                ...
                if not crisis_hits:
                    try:
                        ...
                        state["health_ai_direct_response"] = direct.response_text
        ...
        if state.get("conversation_ended", False):
            ...
            from app.agents.health_ai.background_tasks import (
                trigger_sta_conversation_analysis_background,
            )
            asyncio.create_task(
                trigger_sta_conversation_analysis_background(state.copy())
            )
