...
from app.domains.mental_health.schemas.chat import HealthAIRequest
from app.agents.health_ai_orchestrator_graph import get_health_ai_agent  # compiled once at startup
...
def _build_reasoning_payload(node_name: str, node_state: Dict[str, Any]) -> Dict[str, Any] | None:
    stage_map = {
        "health_ai_decision": "intent_assessment",
    ...
    summary = ""
    if node_name == "health_ai_decision":
...
async def stream_health_ai_execution(
    ...
    try:
        ...
        execution_id = execution_tracker.start_execution(
            graph_id="health_ai_unified_graph",
            agent_name="health_ai",
        ...
        try:
            await emit_agent_event(
                AgentEvent(
                    agent=AgentNameEnum.HEALTH_AI,
                    step="run_started",
                    payload={
                        "user_hash": user_hash,
                        "session_id": initial_state["session_id"],
                        "intent": "health_ai_stream",
        ...
        astream_gen = health_ai_agent.astream(initial_state, config)  # type: ignore
        ...
        logger.warning(
            "Gemini quota exhausted during /health_ai stream: user=%s exec_id=%s model=%s key_idx=%s key_last4=%s retry_after_s=%s",
            current_user.id,
            execution_id,
...
@router.post("/health_ai", dependencies=[Depends(check_rate_limit_dependency)])
async def health_ai_stream_endpoint(
    request: HealthAIRequest,
    http_request: Request,
    ...
    **Use this endpoint for better UX** - shows users what Health-AI is doing in real-time.
    ...
    return StreamingResponse(
        stream_health_ai_execution(request, current_user, db, request_id, http_request),
