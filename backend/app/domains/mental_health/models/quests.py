...
class QuestTemplate(Base):
    ...
    instances: Mapped[list[QuestInstance]] = relationship("QuestInstance", back_populates="template")
...
class QuestInstance(Base):
    ...
    expires_at: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True, default=lambda: datetime.utcnow() + timedelta(days=7))
...
class AttestationRecord(Base):
    ...
    counselor_id: Mapped[Optional[int]] = mapped_column(ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    ...
    counselor: Mapped[Optional["User"]] = relationship("User", foreign_keys=[counselor_id])
