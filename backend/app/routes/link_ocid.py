"""Route for linking an Open Life ID (OCID) to a user's account.

The client completes the OCID OAuth/PKCE flow and receives an `id_token`
(a signed JWT from Open Life). This endpoint:

1. Fetches the Open Life JWKS and verifies the token signature and claims.
...
# ---------------------------------------------------------------------------
_JWKS_SANDBOX = "https://static.openlife.xyz/jwks/jwks-sandbox.json"
_JWKS_LIVE = "https://static.openlife.xyz/jwks/jwks-live.json"
...
async def _fetch_jwks() -> dict:
    """Retrieve the Open Life JSON Web Key Set over HTTPS."""
    url = _jwks_url()
...
async def _verify_id_token(id_token: str) -> dict:
    ...
    try:
        ...
        logger.error("Failed to fetch Open Life JWKS: %s", exc)
        raise HTTPException(
            status_code=502,
            detail="Could not reach the Open Life identity service. Please try again.",
        )
...
async def link_ocid(
    ...
    """Link an Open Life ID and its associated wallet to the current user.

    Accepts the raw `id_token` from the OCID auth flow. The token is verified
    against the Open Life JWKS before any mutations occur.
    """
    ...
    if not eth_address:
        raise HTTPException(
            status_code=422,
            detail="id_token is missing the `eth_address` claim. Ensure this token was issued by Open Life.",
        )
    ...
    if ocid_conflict.scalar_one_or_none():
        raise HTTPException(
            status_code=409,
            detail="This Open Life ID is already linked to another account.",
        )
