...
async def get_overview(
    ...
    if latest_report:
        ...
        if latest_report.trending_topics and isinstance(latest_report.trending_topics, dict):
            ...
            items = topics_list if isinstance(topics_list, list) else []
            trending = [
                TrendingTopic(topic=t.get("topic", ""), count=t.get("count", 0))
                for t in items
                if isinstance(t, dict)
            ]
        elif latest_report.trending_topics and isinstance(latest_report.trending_topics, list):
            trending = [
                TrendingTopic(topic=t.get("topic", ""), count=t.get("count", 0))
                for t in latest_report.trending_topics
                if isinstance(t, dict)
            ]
    else:
