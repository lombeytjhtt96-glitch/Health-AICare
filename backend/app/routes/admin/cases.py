    ...
    **Admin only** - Requires admin role.
    ...
    try:
        ...
        if sla_breached is True:
            stmt = stmt.where(Case.sla_breach_at < datetime.now(timezone.utc))
        ...
        logger.debug(f"DEBUG_CASES: Found {total} total cases before pagination with filters: status={status}, severity={severity}, search={search}")
...
async def update_case_status(
    ...
    **Admin only** - Requires admin role.
    ...
    try:
        ...
        valid_statuses = ['new', 'waiting', 'in_progress', 'resolved', 'closed']
