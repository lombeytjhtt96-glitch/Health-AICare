...
class UpdateActiveModelPayload(BaseModel):
    model: str
...
async def get_api_key_status(
    ...
    return ApiKeyStatusResponse(
        ...
        active_chat_model=str(getattr(llm, "DEFAULT_GEMINI_MODEL", "gemini-2.5-flash")),
        active_chat_provider="gemini",
        supported_chat_models=list(getattr(llm, "GEMINI_FALLBACK_CHAIN", [])),
    )
...
async def update_active_model(
    body: UpdateActiveModelPayload,
    admin_user: Any = Depends(get_admin_user),
) -> ActiveModelStatusResponse:
    """Update the default active AI model used for chat routing at runtime."""
    import app.core.llm as llm
    del admin_user

    # Update only if it exists in the fallback/supported chain
    if body.model in getattr(llm, "GEMINI_FALLBACK_CHAIN", []):
        llm.DEFAULT_GEMINI_MODEL = body.model
