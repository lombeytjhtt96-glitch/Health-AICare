...
async def get_conversation_assessment(
    id_or_conversation_id: str,
    db: AsyncSession = Depends(get_async_db),
    admin_user: User = Depends(get_admin_user),
) -> ConversationRiskAssessmentResponse:
    """Return an assessment by ID or the most recent one for a conversation."""

    # Try lookup by record primary key (id) OR conversation_id
    record = (
        await db.execute(
            select(ConversationRiskAssessment)
            .where(
                or_(
                    ConversationRiskAssessment.id == id_or_conversation_id,
                    ConversationRiskAssessment.conversation_id == id_or_conversation_id
                )
            )
            .order_by(desc(ConversationRiskAssessment.analysis_timestamp))
        )
...
async def trigger_conversation_assessment(
    ...
    try:
        ...
        logger.warning(f"STA assessment failed due to rate limits: {e}")
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS,
            detail=f"Gemini API rate limit exceeded. Please try again later. (Retry after {e.retry_after_s}s)" if getattr(e, 'retry_after_s', None) else "Gemini API rate limit exceeded. Please try again later."
        )
