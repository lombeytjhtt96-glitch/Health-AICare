...
class SeedDatabaseRequest(BaseModel):
    ...
    users_count: int = Field(default=5, description="Number of test users to create")
...
async def seed_database(
    ...
    counselor_data = [
        ...
        },
        {
            ...
            "bio": "Pak Arif berfokus pada psikologi pendidikan dan membantu klien mengatasi stres akademik/kerja, burnout, dan masalah motivasi belajar. Pendekatan beliau ramah dan solution-focused.",
            "education": {
                "degrees": [
    ...
    admin_names = ["Admin Kesehatan Mental", "Koordinator Layanan Konseling"]
    ...
    for i in range(min(request.users_count, len(student_names))):
        ...
        email = f"{sanitized_name}.test.{random_suffix}@example.com"
        ...
        db.add(user)
        created_details.append(f"User: {name} ({email})")
    ...
    logger.info(f"Seeded database: {request.users_count} users, {request.counselors_count} counselors, {request.admins_count} admins")
...
async def seed_screening_profiles(
    ...
    if not candidate_users:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="No test users found. Seed users first from Testing page.",
        )
...
Tujuan utamamu adalah menjadi pendengar yang baik, suportif, hangat, dan tidak menghakimi. Bantu pengguna mengeksplorasi perasaan mereka terkait kehidupan sehari-hari, stres, pertemanan, atau apapun yang ada di pikiran mereka. Validasi emosi mereka, tunjukkan kalau kamu paham dan peduli."""
    ...
    for idx, user_message in enumerate(request.user_messages):
        ...
        try:
            ...
            # Update history (ensure it's a list of dicts for Pydantic validation)
            history = [{"role": h.role, "content": h.content} for h in result.final_history]
...
async def generate_coaching_responses(
    ...
    for scenario in scenarios:
        ...
        try:
            ...
            steps_text = "\n".join([f"{i+1}. {s.title} ({s.duration_min} min)" for i, s in enumerate(tca_response.plan_steps)])
