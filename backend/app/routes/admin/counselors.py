...
router = APIRouter(prefix="/api/v1/admin/counselors", tags=["Admin - Counselors"])
...
# ========================================
# Selectable Counselor Users (Unassigned)
# ========================================

@router.get("/selectable-users", response_model=List[CounselorUser])
async def get_selectable_counselor_users(
    db: AsyncSession = Depends(get_async_db),
    current_user: User = Depends(require_admin)
):
    """
    Get users with role 'counselor' who do not have a profile yet.
    Used for the 'Add Counselor' dropdown selection to prevent 409 Conflict.
    """
    # Find all users with role 'counselor'
    # We use a subquery to find user_ids that already have a CounselorProfile
    subquery = select(CounselorProfile.user_id).where(CounselorProfile.user_id.is_not(None))
    ...
    return [
        {
            "id": user.id,
            "email": decrypt_user_email(user.email) if user.email else None,
            "name": decrypt_user_field(user.name) if user.name else (decrypt_user_email(user.email) if user.email else f"Counselor {user.id}"),
            "avatar_url": None, # Placeholder for backward compatibility
...
async def ensure_counselor_profiles_sync(db: AsyncSession) -> None:
    """
    Ensure counselor users have matching profile records.
    This utility can be called manually or via setup scripts.
    It is no longer called automatically during listing to prevent 
    recreation of deleted profiles.
    """
    try:
...
async def list_counselors(
    ...
    # Removed automatic profile recreation to prevent deleted counselors from reappearing
    # and to ensure the 'Add Counselor' dropdown correctly shows users without profiles.
...
async def create_counselor(
    ...
    if existing:
        # If profile exists, check if it's a 'skeleton' profile (auto-generated)
        # We allow fulfilling the profile if it has no bio or specialization yet.
        if not existing.bio and not existing.specialization:
            ...
            existing = _prepare_counselor_display(existing)
            return CounselorResponse.model_validate(existing)
        
        raise HTTPException(
            status_code=409, 
            detail="Counselor profile already exists for this user and is already populated"
        )
...
async def update_counselor(
    ...
    # Debug: log fields being updated
    logger.info(f"Updating counselor {counselor_id} with fields: {list(update_data.keys())}")
    
    for field, value in update_data.items():
        if hasattr(counselor, field):
            setattr(counselor, field, value)
        elif field == "specialization":
            # Map to both possible model field names to ensure persistence
            if hasattr(counselor, "specialization"):
                setattr(counselor, "specialization", value)
            if hasattr(counselor, "specializations"):
                setattr(counselor, "specializations", value)
