...
_WEEKDAY_MAP = {
    ...
    "sa": "sat",
    "su": "sun",
    "ss": "sun",  # Common typo for Sunday/Sat/Sun
}


def _map_weekday_expression(expr: str) -> str:
    """Map short/abbreviated weekday names to APScheduler-compatible names."""
    if not expr:
        return expr
    ...
    for p in parts:
        ...
        if "-" in p:
            ...
            if len(range_parts) == 2:
                start = _WEEKDAY_MAP.get(range_parts[0].strip(), range_parts[0])
                end = _WEEKDAY_MAP.get(range_parts[1].strip(), range_parts[1])
                mapped_parts.append(f"{start}-{end}")
            else:
                mapped_parts.append(p)
        else:
            mapped_parts.append(_WEEKDAY_MAP.get(p, p))
    
    return ",".join(mapped_parts)


_JOB_DESCRIPTIONS.update({
...
async def toggle_or_reschedule_job(
    ...
    if isinstance(payload, ToggleJobRequest):
        ...
        if payload.day_of_week is not None:
            reschedule_kwargs["day_of_week"] = _map_weekday_expression(payload.day_of_week)

        try:
            scheduler.reschedule_job(job_id, **reschedule_kwargs)
            # Resume if it was paused
            updated_job = scheduler.get_job(job_id)
            if updated_job and updated_job.next_run_time is None:
                scheduler.resume_job(job_id)
        except (ValueError, TypeError) as e:
            logger.warning(f"Invalid rescheduling parameters for job {job_id}: {e}")
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Invalid cron parameters: {str(e)}"
            )
    ...
    if updated_job is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Scheduler job '{job_id}' no longer exists after update."
        )
...
async def trigger_user_checkin(
    ...
    """Manually trigger an immediate check-in for a specific user."""
