...
async def get_profile_overview(
    ...
    return UserProfileOverviewResponse(
        ...
        health_aicare_team_notes=(user.clinical_record.health_aicare_team_notes if user.clinical_record else None),
    )
...
async def update_profile_overview(
    ...
    try:
            ...
            if field in data:
                ...
                if not user.preferences:
                    user.preferences = UserPreferences(
                        ...
                        health_ai_personality="empathetic",
                        health_ai_response_length="balanced"
                    )
        ...
        if clinical_data:
            ...
            if "health_aicare_team_notes" in clinical_data:
                user.clinical_record.health_aicare_team_notes = _normalize_optional_string(clinical_data.get("health_aicare_team_notes"))
                updated = True
    ...
    # Reload user with normalized relations to avoid lazy-loading greenlet errors after refresh
    refreshed_user = (
        await db.execute(
            select(User)
            .options(
    ...
    return await get_profile_overview(db=db, current_user=refreshed_user)
...
# SIMASTER Import Endpoint removed to align with general target audience.
