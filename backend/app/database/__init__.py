...
DATABASE_URL = os.getenv("DATABASE_URL", "sqlite+aiosqlite:///./health_ai.db")
...
if DATABASE_URL.startswith("postgresql+asyncpg://"):
    ...
    async_engine = create_async_engine(
        DATABASE_URL,
        echo=False,  # Set logger level to INFO for SQL logging instead of using echo=True
        future=True,
...
async def get_async_db() -> AsyncGenerator[AsyncSession, None]:
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            await asyncio.shield(session.rollback())
            # Don't log HTTPExceptions as database errors (they are client-side or expected flow errors)
