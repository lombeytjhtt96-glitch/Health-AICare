...
from app.agents.health_ai_orchestrator_graph import create_health_ai_agent_with_checkpointing

# Create agent with conversation memory
memory = MemorySaver()
health_ai_agent = create_health_ai_agent_with_checkpointing(db, checkpointer=memory)
...
print(result["response_source"])  # "health_ai_direct" or "agents"
```

**LangGraph Components:**
- **StateGraph**: Typed state (`HealthAIOrchestratorState`, `SafetyAgentState`, `IAState`)
- **Nodes**: `health_ai_decision_node`, `triage_node`, `generate_plan_node`, etc.
...
Refer to `HEALTH_AI_META_AGENT_ARCHITECTURE.md`, `PROJECT_SINGLE_SOURCE_OF_TRUTH.md`, and `docs/langgraph-phase5-complete.md`.
