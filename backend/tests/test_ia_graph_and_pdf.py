...
@pytest.mark.asyncio
async def test_ingest_query_node_validates_range() -> None:
    ...
    out = await ingest_query_node(state)
    assert out.get("query_validated") is True


@pytest.mark.asyncio
async def test_ingest_query_node_rejects_large_range() -> None:
    from app.agents.ia.ia_graph import ingest_query_node
...
async def test_ia_graph_service_execute(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # Mock get_ia_graph instead of create_ia_graph
