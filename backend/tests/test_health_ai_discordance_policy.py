...
from app.agents.health_ai.decision_node import (
...
async def test_apply_screening_discordance_policy_promotes_to_tca(monkeypatch: pytest.MonkeyPatch) -> None:
    import app.agents.health_ai.screening_awareness as screening_awareness
    ...
    await _apply_screening_discordance_policy(
        cast(HealthAIOrchestratorState, state),
        "user",
        db=cast(Any, object()),
    )
