...
async def create_user():
    async with async_session_maker() as db:
        user = User(
            email="finance@health-aicare.org",
...
curl -X POST http://localhost:8001/api/v1/auth/login \
  ...
  -d "username=admin@health-aicare.org&password=admin123"
...
## 🔗 Integration with Main Health-AICare Database
...
class Settings(BaseSettings):
    ...
    MAIN_DB_URL: str = "..."   # Main Health-AICare DB
```
...
Create REST API in main Health-AICare backend:
