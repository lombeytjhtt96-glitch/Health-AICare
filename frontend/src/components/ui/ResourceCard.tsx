...
export interface ResourceCardProps {
    ...
    notes?: string;
    logoUrl?: string;
    category?: 'Local' | 'National' | 'International'; // Optional category for future filtering
}
        ...
        <div className="p-4 sm:p-5 bg-linear-to-br from-white/5 via-white/10 to-white/5 rounded-xl border border-white/10 shadow-lg hover:border-white/20 transition-all duration-300 ease-in-out flex flex-col h-full">
            ...
            {targetAudience && (
                <div className="flex items-center text-xs text-gray-400 mb-2">
                    <FiUsers className="mr-2 shrink-0 text-health-blue-light" /> For: {targetAudience}
                </div>
            )}
                ...
                <div className="mb-3">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                        {servicesOffered.map(service => (
                            <span key={service} className="px-2.5 py-1 text-[0.7rem] bg-health-gold text-health-blue-light rounded-full border border-health-blue-light/30 shadow-sm">
                                {service}
                            </span>
                        ))}
                    </div>
