          ...
          >
            ...
            {viewMode === 'minimized' ? (
              ...
              >
                <button
                  onClick={() => setViewMode('compact')}
                  className="w-12 h-12 rounded-xl bg-health-gold/10 flex items-center justify-center hover:bg-health-gold/20 transition-colors group"
                  title="Buka Rencana Intervensi"
                >
                  <ListChecks className="w-5 h-5 text-health-gold group-hover:scale-110 transition-transform" />
                </button>
                
                {totalPlans > 0 && (
                  <div className="flex flex-col items-center gap-1">
                    <span className="text-xs font-bold text-health-gold">{totalPlans}</span>
              ...
              <>
                ...
                <div className="flex items-center justify-between px-4 py-3 border-b border-white/10 bg-white/2 shrink-0">
                  <div className="flex items-center gap-2.5">
                    <div className="w-8 h-8 rounded-lg bg-health-gold/10 flex items-center justify-center">
                      <ListChecks className="w-4 h-4 text-health-gold" />
                    </div>
                    <div>
                ...
                {totalPlans > 0 && viewMode === 'compact' && (
                  <div className="px-4 py-2 border-b border-white/5 bg-health-gold/5">
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="text-[10px] text-white/50">Progress Keseluruhan</span>
                      <span className="text-[10px] font-bold text-health-gold">{overallProgress}%</span>
                    </div>
                    <div className="h-1 bg-white/10 rounded-full overflow-hidden">
                      <motion.div
                        ...
                        className="h-full bg-linear-to-r from-health-gold to-health-gold-light rounded-full"
                      />
                    </div>
                  </div>
                )}
                ...
                <div className="flex-1 overflow-y-auto px-3 py-3 space-y-2.5 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
                  ...
                  {isLoading && (
                    <div className="flex flex-col items-center justify-center py-12">
                      <motion.div
                        ...
                        className="mb-3"
                      >
                        <div className="w-10 h-10 rounded-xl bg-health-gold/10 flex items-center justify-center">
                          <RefreshCw className="w-5 h-5 text-health-gold" />
                        </div>
                      </motion.div>
                  ...
                  {error && !isLoading && (
                    <div className="flex flex-col items-center justify-center py-12 text-center">
                      ...
                      <button
                        onClick={() => refetch()}
                        className="px-3 py-1.5 bg-health-gold/10 border border-health-gold/30 text-health-gold text-[10px] font-medium rounded-lg hover:bg-health-gold/20 transition-all"
                      >
                        Coba Lagi
                      </button>
                    </div>
