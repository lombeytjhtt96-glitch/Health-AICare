...
interface HealthAIThinkingIndicatorProps {
...
const AGENT_CONFIG: Record<string, { icon: React.ElementType; color: string; bgColor: string; borderColor: string }> = {
  ...
  HEALTH_AI: { icon: Brain, color: 'text-health-gold', bgColor: 'bg-health-gold/20', borderColor: 'border-health-gold/30' },
};

export function HealthAIThinkingIndicator({ status, isActive }: HealthAIThinkingIndicatorProps) {
  if (!isActive || !status) {
    return null;
  }

  const getAgentConfig = () => {
    const agentName = status.agent?.toUpperCase() || 'HEALTH_AI';
    return AGENT_CONFIG[agentName] || AGENT_CONFIG.HEALTH_AI;
  };
 ...
 */
export function HealthAIThinkingCompact({ message }: { message?: string }) {
  return (
    <div className="flex items-center gap-2.5 px-3 py-2">
      ...
      <div className="flex items-center gap-0.5 ml-auto">
        {[0, 1, 2].map((i) => (
          <motion.div
            key={i}
            className="h-1.5 w-1.5 rounded-full bg-health-gold"
            animate={{
              scale: [1, 1.3, 1],
              opacity: [0.4, 1, 0.4],
            }}
