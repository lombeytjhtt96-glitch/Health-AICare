...
import { HealthAISchedulingWidget } from './HealthAISchedulingWidget';
import { HealthAIThinkingCompact } from './HealthAIThinkingIndicator';
...
if (typeof window !== 'undefined') {
  ...
  audioCache.health_ai = new Audio('/sounds/message_bubble_health_ai.mp3');
  // Preload the audio
  audioCache.user.load();
  audioCache.health_ai.load();
}
...
export function MessageBubble({
  ...
  useEffect(() => {
    ...
    if (!message.isLoading && messageSoundsEnabled && !hasPlayedRef.current && !message.isContinuation) {
      ...
      const cachedAudio = message.role === 'user' ? audioCache.user : audioCache.health_ai;
      if (cachedAudio) {
  ...
  const shouldShowSchedulingWidget = React.useMemo(() => {
    ...
    const intent = message.health_aiMetadata?.intent?.toLowerCase() || '';
    const actions = message.health_aiMetadata?.actions_taken || [];
  ...
  }, [isUser, message.isLoading, message.appointment, message.health_aiMetadata, widgetAppointment]);
    ...
    return (
      <>
        <div className={cn(
          ...
          // Headers styling
          'prose-headings:font-semibold prose-headings:text-health-gold prose-headings:mt-3 prose-headings:mb-1.5',
          'prose-h1:text-base prose-h2:text-sm prose-h3:text-sm',
          // Strong/Bold styling
          'prose-strong:text-health-gold prose-strong:font-semibold',
          // Code/Pre styling
          'prose-code:text-health-gold/90 prose-code:bg-white/5 prose-code:px-1 prose-code:rounded prose-code:text-xs',
          // Blockquote styling
          'prose-blockquote:border-l-2 prose-blockquote:border-health-gold/50 prose-blockquote:pl-3 prose-blockquote:italic prose-blockquote:text-white/70',
          // Text colors
          ...
          isUser ? 'prose-a:text-health-gold hover:prose-a:text-health-gold/80' : 'prose-a:text-health-gold hover:prose-a:text-health-gold/80'
        )}>
          <ReactMarkdown
            components={{
              a: ({ ...props }) => <a {...props} target="_blank" rel="noopener noreferrer" className="break-all" />,
              // Enhanced heading rendering
              h1: ({ children, ...props }) => <h2 {...props} className="text-base font-semibold text-health-gold mt-3 mb-1.5">{children}</h2>,
              h2: ({ children, ...props }) => <h3 {...props} className="text-sm font-semibold text-health-gold mt-2.5 mb-1">{children}</h3>,
              h3: ({ children, ...props }) => <h4 {...props} className="text-sm font-medium text-health-gold/90 mt-2 mb-1">{children}</h4>,
              // Better list rendering
              ul: ({ children, ...props }) => <ul {...props} className="my-2 pl-4 space-y-1 list-disc marker:text-health-gold/60">{children}</ul>,
              ol: ({ children, ...props }) => <ol {...props} className="my-2 pl-4 space-y-1 list-decimal marker:text-health-gold/60">{children}</ol>,
              ...
              strong: ({ children, ...props }) => <strong {...props} className="font-semibold text-health-gold">{children}</strong>,
            ...
            {canShowActions && (
              <div className="flex items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100 group-focus-within:opacity-100">
                <button
                  type="button"
                  onClick={handleCopy}
                  className="inline-flex h-7 w-7 items-center justify-center rounded-full border border-white/10 bg-black/20 text-white/70 backdrop-blur hover:bg-black/30 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-health-gold/50"
                  aria-label="Salin pesan"
                >
                  {copied ? <Check className="h-4 w-4 text-health-gold" /> : <Copy className="h-4 w-4" />}
                </button>
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className={cn(
                    "inline-flex h-7 w-7 items-center justify-center rounded-full border border-white/10 bg-black/20 text-white/70 backdrop-blur hover:bg-black/30 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-health-gold/50",
