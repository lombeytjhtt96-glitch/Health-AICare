...
interface HealthAILoadingBubbleProps {
  activeAgents?: string[];
...
const AGENT_CONFIG = {
  ...
  HEALTH_AI: { 
    ...
    borderColor: 'border-health-gold/30',
    action: 'Memproses...' 
  },
};
...
export function HealthAILoadingBubble({ activeAgents = [], currentThinking = null, className = '' }: HealthAILoadingBubbleProps) {
    ...
    >
      ...
      <div className="flex flex-col items-start">
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl rounded-bl-sm border border-white/10 px-3 py-2 flex items-center gap-3 min-w-45">
          ...
          <div className={`shrink-0 w-6 h-6 rounded-full flex items-center justify-center ${currentAgent?.bgColor || 'bg-health-gold/20'} ${currentAgent?.borderColor || 'border-health-gold/30'} border`}>
            <motion.div
              animate={{ scale: [1, 1.1, 1] }}
              transition={{ duration: 1.5, repeat: Infinity }}
            >
              <AgentIcon className={`w-3.5 h-3.5 ${currentAgent?.color || 'text-health-gold'}`} />
            </motion.div>
          </div>
