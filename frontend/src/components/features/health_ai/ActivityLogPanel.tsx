...
const EVENT_CONFIG: Record<ActivityType, { icon: React.ElementType; color: string }> = {
  ...
  tool_use: { icon: Zap, color: 'text-health-gold' },
  tool_end: { icon: CheckCircle, color: 'text-green-400' },
  reasoning_trace: { icon: Cpu, color: 'text-health-gold' },
  ...
  return (
    <div
      className={`rounded-xl border ${
        isLatest ? 'border-health-gold/30 bg-health-gold/5' : 'border-white/10 bg-white/3'
      } overflow-hidden`}
    >
      <button
        type="button"
