...
const AGENT_CONFIG: Record<string, { color: string; bgColor: string; borderColor: string; label: string }> = {
  ...
  HEALTH_AI: { color: 'text-health-gold',   bgColor: 'bg-health-gold/20',   borderColor: 'border-health-gold/30',   label: 'HEALTH_AI'},
};
              ...
              {activeAgents.length > 0 ? (
                <div className="flex -space-x-1">
                  {activeAgents.map((agent, i) => {
                    const conf = AGENT_CONFIG[agent.toUpperCase()] || AGENT_CONFIG.HEALTH_AI;
                    return (
                      <div 
                        key={`${agent}-${i}`}
                ...
                <div className="w-4 h-4 rounded-full flex items-center justify-center text-[8px] font-bold border border-black bg-health-gold/20 text-health-gold">
                  A
                </div>
              )}
          ...
          >
            <AnimatePresence initial={false}>
              {displaySteps.map((step, index) => {
                ...
                const conf = AGENT_CONFIG[step.agent] || AGENT_CONFIG.HEALTH_AI;
