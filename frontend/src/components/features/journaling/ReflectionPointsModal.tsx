    ...
    if (isLoading) {
        return (
            <div className={`p-6 text-white ${className}`}>
                <h3 className="text-lg font-semibold text-health-gold mb-3">Reflection Insights</h3>
                <div className="flex justify-center items-center h-40">
                    <FiLoader className="animate-spin text-health-gold-light" size={24} />
    ...
    return (
        <div className={`p-6 text-white ${className}`}>
            <div className="flex justify-between items-center mb-3">
                <h3 className="text-lg font-semibold text-health-gold">
                    Reflection Insights
            ...
            {reflectionPoints.length > 0 ? (
                <div className="max-h-[60vh] overflow-y-auto pr-2"> {/* Adjust max-h as needed */}
                    ...
                    <ul className="space-y-3">
                        {reflectionPoints.map(reflection => (
                            <li 
                                key={reflection.id} 
                                className="p-3 bg-health-blue-dark/50 rounded-lg border border-health-blue-light/40"
                            >
                                <div className="flex items-start">
                                    <FiMessageCircle className="text-health-gold-light mr-3 mt-1 flex-shrink-0" size={18} />
                ...
                <div className="text-center py-8">
                    <FiInfo size={30} className="mx-auto text-health-blue-light mb-3" />
