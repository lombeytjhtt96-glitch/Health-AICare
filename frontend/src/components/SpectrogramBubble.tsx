...
const SpectrogramBubble: React.FC<SpectrogramBubbleProps> = ({ isActive, data }) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-4 bottom-2 h-24 rounded-2xl bg-linear-to-t from-health-gold/10 via-transparent to-transparent blur-xl" aria-hidden="true" />
      <div className="relative z-10 flex h-full w-full items-end justify-center gap-[6px]">
        {bars.map((value, index) => {
          ...
          return (
            <span
              key={`${index}-${value}`}
              className="flex-1 rounded-full bg-linear-to-t from-health-gold/15 via-health-gold/35 to-white/90 transition-[height,opacity,filter] duration-150 ease-out"
              style={{
                height: `${height}%`,
                opacity,
