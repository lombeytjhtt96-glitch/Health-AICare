...
const pillColor: Record<string, string> = {
    ...
    health_ai: "bg-purple-500/20 text-purple-300 border border-purple-400/40",
...
export default function HealthAIChatWidget() {
    ...
    const submitMessage = async (text: string, correlationId: string) => {
        ...
        try {
            ...
            const res = await fetch(`${config?.apiBase}/health_ai`, {
                method: 'POST',
                headers: {
            ...
            setMessages(prev => prev.map(m => (m.correlationId === correlationId && m.role === 'assistant')
                ? ({
                    ...
                    pending: false,
                    resolvedAgent: metadata.response_source || 'health_ai',
                    agentsInvoked,
                    riskLevel,
                    riskScore,
                    interventionPlan,
