...
export function useHealthAIChat({ 
  sessionId, 
  ...
  const handleSendMessage = useCallback(
    async (message?: string) => {
      ...
      if (isLoading || health_aiLoading) return;
      ...
      try {
        ...
        setLastMetadata(health_aiResponse.metadata);
        ...
        if (health_aiResponse.isFallback && health_aiResponse.metadata.retry_after_ms) {
          setRetryCooldownMs(health_aiResponse.metadata.retry_after_ms);
        }

        const fallbackProps: Partial<Message> = health_aiResponse.isFallback
          ? {
              isError: true,
              retryAfterMs: health_aiResponse.metadata.retry_after_ms ?? 0,
              fallbackType: health_aiResponse.metadata.fallback_type,
            }
          : {};
        ...
        setMessages((prev) => {
          if (hasStreamedPartials) {
            ...
            if (bubbleIndex !== -1) {
              ...
              next[bubbleIndex] = {
                ...
                isLoading: false,
                updated_at: new Date().toISOString(),
                health_aiMetadata: health_aiResponse.metadata,
          ...
          const newMessages: Message[] = sections.map((section, idx) => ({
            ...
            health_aiMetadata: idx === sections.length - 1 ? health_aiResponse.metadata : undefined,
  ...
  const isChatLoading = isLoading || health_aiLoading;
