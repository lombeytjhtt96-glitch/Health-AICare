...
import { getCounselorAgentDecisions } from '@/services/counselorAgentDecisionsApi';
...
export default function CounselorAgentDecisionsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await getCounselorAgentDecisions(60, 0);
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-end">
        <button
          onClick={() => void load()}
          disabled={loading}
          className="rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-sm text-white hover:bg-white/15 disabled:opacity-50"
        >
              ...
              {items.map((item) => (
                <tr key={item.id} className="border-b border-white/5 align-top">
                  <td className="px-3 py-2">
                    ...
                    <div className="text-xs text-white/50">user={item.user_id ?? '-'} · {item.policy_decision}</div>
                  </td>
                  <td className="max-w-80 px-3 py-2 text-xs text-white/70">
