...
import { useHealthAIChat, type ToolActivityLog } from '@/hooks/useHealthAIChat';
import { useProfilePicture } from '@/hooks/useProfilePicture';
import { ChatWindow } from '@/components/features/chat/ChatWindow';
import { ChatInput } from '@/components/features/chat/ChatInput';
import { HEALTH_AI_MEMORY_NOTE } from '@/constants/chat';
import { useInterventionPlans } from '@/hooks/useInterventionPlans';
import { HealthAILoadingBubble } from '@/components/features/health_ai/HealthAILoadingBubble';
import {
  RiskLevelIndicator,
...
export default function AikaEnhancedPage() {
  ...
  // Activity logging - must be declared before useHealthAIChat to pass addActivity
  const {
    activities,
    latestActivity,
    isReceiving,
  ...
  const {
    ...
    sessionId: 'health_ai-session-' + new Date().getTime(),
  ...
  useEffect(() => {
    const saved = window.localStorage.getItem('health_ai_show_thinking_trace');
    if (saved === '0') {
  ...
  useEffect(() => {
    window.localStorage.setItem('health_ai_show_thinking_trace', showThinkingTrace ? '1' : '0');
  }, [showThinkingTrace]);
