...
export function AgenticArchitectureGraph({ onNodeClick, healthData }: AgenticArchitectureGraphProps) {
    ...
    const initialNodes: Node[] = [
        ...
        { id: 'health_ai', type: 'agent', position: { x: 340, y: 110 }, data: { label: 'HEALTH_AI', statusLabel: 'ORCHESTRATOR', ...getGraphData('orchestrator') } },
    ...
    const initialEdges: Edge[] = [
        createBrutalistEdge('e-u-a', 'user', 'health_ai', '#fff', 'INPUT'),
        createBrutalistEdge('e-a-t', 'health_ai', 'tca', '#eab308', 'MODERATE'),
        createBrutalistEdge('e-a-p', 'health_ai', 'parallel_crisis', '#ef4444', 'CRITICAL'),
        createBrutalistEdge('e-a-i', 'health_ai', 'ia', '#a855f7', 'ANALYTICS'),
        createBrutalistEdge('e-a-e', 'health_ai', 'end', '#fff', 'DIRECT', true),
        ...
        { ...createBrutalistEdge('e-a-b', 'health_ai', 'sta_bg', '#fff', 'ASYNC', true), targetHandle: 'target-left' },
    ];
    ...
    useMemo(() => {
        setNodes((nds) =>
            nds.map((node) => {
                ...
                const graphName = node.id === 'health_ai' ? 'orchestrator' : node.id === 'sta_bg' ? 'sta' : node.id;
