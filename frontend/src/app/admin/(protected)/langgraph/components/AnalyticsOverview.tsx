...
export function AnalyticsOverview({ days }: AnalyticsOverviewProps) {
  ...
  const totalPromptTokens = data.data.total_prompt_tokens ?? 0;
  const totalCompletionTokens = data.data.total_completion_tokens ?? 0;
  const totalCostUsd = data.data.total_cost_usd ?? 0.0;
  const successRatePercent = data.data.success_rate_percent ?? 0.0;

  const totalTokens = totalPromptTokens + totalCompletionTokens;
  const errorRate = 100 - successRatePercent;
  ...
  const expensiveAgentCost = (totalCostUsd * 0.65).toFixed(2); // Synthetic distribution for demo visual spec
      ...
      <div className="flex items-center justify-between min-w-max gap-12">
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-2xl text-white font-bold">${totalCostUsd.toFixed(2)}</span>
        ...
        <div className="flex flex-col gap-1 border-l border-white/10 pl-6">
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-xs text-white/60">({(totalPromptTokens / 1000).toFixed(1)}k IN / {(totalCompletionTokens / 1000).toFixed(1)}k OUT)</span>
          </div>
        </div>
