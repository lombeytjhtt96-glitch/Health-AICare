...
You've successfully created the $CARE token for Health-AICare on SOMNIA blockchain! 🎉

The token is now ready to:
- [Done] Reward users for mental health activities
- [Done] Enable real-world purchases (vouchers, tickets)
- [Done] Power the entire Health-AICare economy
...
sequenceDiagram
 ...
 participant HEALTH_AI as TCA Agent
 participant BE as Backend System
 participant SC as Smart Contract (EDU Chain)
 participant W as Blockchain Wallet

 S->>HEALTH_AI: Completes Breathing Exercise
 HEALTH_AI->>BE: Report Exercise Completion
