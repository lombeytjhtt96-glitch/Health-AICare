...
sequenceDiagram
 ...
 FE->>API: POST /api/v1/health_ai (message + session)
 API->>DB: Fetch user profile & context
 API->>HEALTH_AI: Invoke orchestrator graph (compiled singleton)
 HEALTH_AI->>HEALTH_AI: health_ai_decision_node
 HEALTH_AI->>GEM: Classify intent & real-time risk
 GEM-->>HEALTH_AI: Risk level + intent
 alt Risk ≥ HIGH
 HEALTH_AI->>HEALTH_AI: parallel_crisis_node (TCA + CMA async fan-out)
 else Risk = MODERATE
 HEALTH_AI->>HEALTH_AI: execute_sca node (TCA only)
 else Risk = LOW
 HEALTH_AI->>GEM: Generate empathetic reply (ReAct loop)
 end
 HEALTH_AI-->>API: Final synthesised response
 API-->>FE: Stream response via SSE
 FE-->>S: Response appears token-by-token
 API->>DB: Persist message & risk data (async)
 HEALTH_AI-->>STA: Trigger deep analysis (async, non-blocking)
